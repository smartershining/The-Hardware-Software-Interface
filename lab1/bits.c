...
int addOK(int x, int y) {
  ...
  return !((!signX & !signY & signSum) | (signX & signY & !signSum)) ;
}
// Rating: 4
/* 
...
int bang(int x) {
  int sign = (x >> 31) & 1;
  int reverse = (~x) + 1;
  int reverseSign = (reverse >> 31) & 1;
  int notZero = sign | reverseSign;
  return notZero ^ 1;
}
...
int isPower2(int x) {
 int sign = x >> 31;
 int minus = ~0;
 int minusOne = x + minus;
 sign = ~sign;
 return (!(x & minusOne)) & sign & (!!x); 
 return 2;
}
